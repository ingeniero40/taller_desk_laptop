...
class ReportManager:
    ...
    def get_monthly_financial_report(self) -> Dict[str, Any]:
        ...
        current_stats = self.analytics_repo.get_revenue_stats(start_current, end_current)
        previous_stats = self.analytics_repo.get_revenue_stats(start_last, end_last)
        
        growth = 0.0
        if previous_stats["total_collected"] > 0:
            growth = ((current_stats["total_collected"] / previous_stats["total_collected"]) - 1) * 100
